...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $login_type = $_POST['login_type']; // Determine if it's an admin or user login
    ...
    if ($login_type === 'admin') {
        ...
        if (handleUserLogin($username, $password)) {
            ...
            header("Location: signInPage.php?error=Invalid%20username%20or%20password");
            exit();
        }
    }
}
...
function handleAdminLogin($username, $password) {
    ...
    if ($username === $adminUsername && $password === $adminPassword) {
        ...
        header("Location: ../public/menuAdmin.html");
        exit();
    } else {
